...
## Planner Compliance (v2.20+)

**Always include** `planner_compliance` at the root level of generated plans. This enables strict validation when `strict_rubric: true` is set in `.conductor/config.yaml`.

```yaml
# At root level, after conductor: block
planner_compliance:
...
| Field | Purpose |
|-------|---------|
| `planner_version` | Skill version that generated the plan |
| `strict_enforcement` | Enable strict rubric validation |
| `required_features` | Features the plan must include for validation |
...
- Data flow registry completeness

**When `strict_rubric: true` + plan has `planner_compliance`:**
- Conductor validates key_points ↔ success_criteria alignment
- Mismatches cause validation warnings/errors
- Plans without `planner_compliance` skip rubric validation
