...
// TestDefaultGuardConfig verifies default Guard configuration values
func TestDefaultGuardConfig(t *testing.T) {
	...
	if cfg.Mode != GuardModeWarn {
		t.Errorf("Mode = %q, want %q", cfg.Mode, GuardModeWarn)
	}
	if cfg.ProbabilityThreshold != 0.7 {
		t.Errorf("ProbabilityThreshold = %f, want 0.7", cfg.ProbabilityThreshold)
	}
	if cfg.ConfidenceThreshold != 0.7 {
		t.Errorf("ConfidenceThreshold = %f, want 0.7", cfg.ConfidenceThreshold)
	}
	if cfg.MinHistorySessions != 5 {
		t.Errorf("MinHistorySessions = %d, want 5", cfg.MinHistorySessions)
	}
}

// TestDefaultConfigIncludesGuard verifies that default config includes Guard section
func TestDefaultConfigIncludesGuard(t *testing.T) {
	cfg := DefaultConfig()

	if cfg.Guard.Enabled != false {
		t.Errorf("Guard.Enabled = %v, want false", cfg.Guard.Enabled)
	}
	if cfg.Guard.Mode != GuardModeWarn {
		t.Errorf("Guard.Mode = %q, want %q", cfg.Guard.Mode, GuardModeWarn)
	}
}

// TestLoadGuardConfigFullSettings verifies loading complete Guard configuration
func TestLoadGuardConfigFullSettings(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "config.yaml")
...
`
	...
	if cfg.Guard.Enabled != true {
		t.Errorf("Guard.Enabled = %v, want true", cfg.Guard.Enabled)
	}
	if cfg.Guard.Mode != GuardModeBlock {
		t.Errorf("Guard.Mode = %q, want %q", cfg.Guard.Mode, GuardModeBlock)
	}
	if cfg.Guard.ProbabilityThreshold != 0.8 {
		t.Errorf("Guard.ProbabilityThreshold = %f, want 0.8", cfg.Guard.ProbabilityThreshold)
	}
	if cfg.Guard.ConfidenceThreshold != 0.85 {
		t.Errorf("Guard.ConfidenceThreshold = %f, want 0.85", cfg.Guard.ConfidenceThreshold)
	}
	if cfg.Guard.MinHistorySessions != 10 {
		t.Errorf("Guard.MinHistorySessions = %d, want 10", cfg.Guard.MinHistorySessions)
	}
}

// TestLoadGuardConfigPartialSettings verifies partial Guard configuration merges with defaults
func TestLoadGuardConfigPartialSettings(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "config.yaml")
...
`
	...
	if cfg.Guard.Mode != GuardModeAdaptive {
		t.Errorf("Guard.Mode = %q, want %q", cfg.Guard.Mode, GuardModeAdaptive)
	}
	// These should still have default values
	if cfg.Guard.ProbabilityThreshold != 0.7 {
		t.Errorf("Guard.ProbabilityThreshold = %f, want 0.7 (default)", cfg.Guard.ProbabilityThreshold)
	}
	if cfg.Guard.ConfidenceThreshold != 0.7 {
		t.Errorf("Guard.ConfidenceThreshold = %f, want 0.7 (default)", cfg.Guard.ConfidenceThreshold)
	}
	if cfg.Guard.MinHistorySessions != 5 {
		t.Errorf("Guard.MinHistorySessions = %d, want 5 (default)", cfg.Guard.MinHistorySessions)
	}
}

// TestLoadGuardConfigDisabledByDefault verifies Guard is disabled by default
func TestLoadGuardConfigDisabledByDefault(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "config.yaml")

	// Config with no guard section
	configContent := `max_concurrency: 5
log_level: info
`
	...
	if cfg.Guard.Enabled != false {
		t.Errorf("Guard.Enabled = %v, want false (default)", cfg.Guard.Enabled)
	}
	if cfg.Guard.Mode != GuardModeWarn {
		t.Errorf("Guard.Mode = %q, want %q (default)", cfg.Guard.Mode, GuardModeWarn)
	}
}

// TestGuardConfigValidation verifies Guard configuration validation
func TestGuardConfigValidation(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid guard config",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeBlock
				cfg.Guard.ProbabilityThreshold = 0.8
				cfg.Guard.ConfidenceThreshold = 0.8
				cfg.Guard.MinHistorySessions = 5
			},
			wantError: false,
		},
		{
			...
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardMode("invalid")
			},
			wantError: true,
			errorMsg:  "guard.mode must be one of: block, warn, adaptive",
		},
		{
			name: "probability threshold too high",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.ProbabilityThreshold = 1.5
			},
			wantError: true,
			errorMsg:  "guard.probability_threshold must be between 0 and 1",
		},
		{
			name: "probability threshold negative",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.ProbabilityThreshold = -0.1
			},
			wantError: true,
			errorMsg:  "guard.probability_threshold must be between 0 and 1",
		},
		{
			...
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.ConfidenceThreshold = 1.1
			},
			wantError: true,
			errorMsg:  "guard.confidence_threshold must be between 0 and 1",
		},
		{
			...
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.ConfidenceThreshold = -0.5
			},
			wantError: true,
			errorMsg:  "guard.confidence_threshold must be between 0 and 1",
		},
		{
			name: "min history sessions negative",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.MinHistorySessions = -1
			},
			wantError: true,
			errorMsg:  "guard.min_history_sessions must be >= 0",
		},
		{
			name: "disabled guard with invalid values",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = false
				cfg.Guard.Mode = GuardMode("invalid")
				cfg.Guard.ProbabilityThreshold = 2.0
			},
			wantError: false, // Validation should not run when disabled
		},
		{
			name: "all valid modes - block",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeBlock
			},
		...
		{
			name: "all valid modes - adaptive",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeAdaptive
			},
			wantError: false,
		},
		{
			name: "boundary values - 0.0 thresholds",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.ProbabilityThreshold = 0.0
				cfg.Guard.ConfidenceThreshold = 0.0
			},
			wantError: false,
		},
		{
			name: "boundary values - 1.0 thresholds",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.ProbabilityThreshold = 1.0
				cfg.Guard.ConfidenceThreshold = 1.0
			},
			wantError: false,
		},
		{
			name: "zero min history sessions",
			modify: func(cfg *Config) {
				cfg.Guard.Enabled = true
				cfg.Guard.Mode = GuardModeWarn
				cfg.Guard.MinHistorySessions = 0
			},
			wantError: false,
		},
	}
...
// TestGuardConfigAllModes verifies all valid Guard modes
func TestGuardConfigAllModes(t *testing.T) {
	modes := []GuardMode{
		GuardModeBlock,
		GuardModeWarn,
		GuardModeAdaptive,
	...
	for _, mode := range modes {
		t.Run(string(mode), func(t *testing.T) {
			...
			configContent := fmt.Sprintf(`guard:
  enabled: true
  mode: %s
`, mode)
			...
			if cfg.Guard.Mode != mode {
				t.Errorf("Guard.Mode = %q, want %q", cfg.Guard.Mode, mode)
			}

			if err := cfg.Validate(); err != nil {
				t.Errorf("Validate() failed for mode %q: %v", mode, err)
			}
		})
	}
}
