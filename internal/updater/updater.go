...
func UpdateTaskFeedback(planPath string, taskNumber string, attempt *ExecutionAttempt) error {
	...
	// Ensure write is fully sync'd before releasing lock.
	// This prevents another goroutine from reading a partially-written file.
	if f, err := os.Open(planPath); err == nil {
		_ = f.Sync()
		_ = f.Close()
	}
